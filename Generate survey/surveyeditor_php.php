...
//saving image and theme components from user browse-----------------------------------------------------------------------------------------------
if(isset($_POST['submit_sbgfile']))
{
	$outer_div=0;$inner_div=0;$outer_div_color="white";$inner_div_color="lightgray";$theme_black=0;
	//getting value
	if(isset($_POST['enable_theme_black']))$theme_black=1;
	if(isset($_POST['enable_Outer_div']))$outer_div=1;
	if(isset($_POST['enable_Inner_div']))$inner_div=1;
	if($_POST['Outer_div_color'])$outer_div_color=$_POST['Outer_div_color'];else $outer_div_color="white";
	if($_POST['Inner_div_color'])$inner_div_color=$_POST['Inner_div_color'];else $inner_div_color="lightgray";
	...
	if($run)
	{
		...
		if(file_exists($file))
		{
			if($_FILES['sbgfile']['name'])
			{
			unlink($file);
			$imagename="";$imagename=$_FILES['sbgfile']['name'];
			}
			...
			$table_name="userid_".$_SESSION['user_id'];
			$current_survey_id=$_SESSION['current_survey_name_id'];
			$sql="UPDATE $table_name SET SurveyBackgroundPic='$imagename',OuterDiv='$outer_div',InnerDiv='$inner_div',OuterDivColor='$outer_div_color',InnerDivColor='$inner_div_color',ThemeBlack='$theme_black' WHERE ID='$current_survey_id'";
			$file='dataimg/'.$imagename;
			$run=mysqli_query($con,$sql);
			if($run)
			{
...
if(isset($_POST[$current_ques_id1]))
{
    ...
    if(mysqli_num_rows($run)>0)
    {
        ...
        $sql="UPDATE $table_name SET Ques='$ques',FontFamilyQues='$font', Ques_Order='$ques_order',Page='$page', Multiple='$multiple', Required='$required',Heading='$heading', Bold='$bold',Italic='$italic',Underline='$underline',Text_SizeQues='$text_size',Answer_Type='$answer_type',Answer_Options='$answer_options', Save_Ques='0' WHERE ID='$ques_id'";
		$run=mysqli_query($con1,$sql);
		if($run)
			{
			    //inserting options
